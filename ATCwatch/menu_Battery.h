...
class BatteryScreen : public Screen
{
  public:
    virtual void pre()
    {
      ...
      lv_obj_align(label, LV_ALIGN_IN_TOP_LEFT, 0, 0);

      label_battery = lv_label_create(lv_scr_act());
      lv_label_set_text(label_battery, "---%");
      lv_obj_align(label_battery, LV_ALIGN_IN_TOP_LEFT, 10, 30);
      ...
      label_battery_big = lv_label_create(lv_scr_act());
      lv_obj_set_style( label_battery_big, &st );
      lv_label_set_text(label_battery_big, "---%");
      lv_obj_align(label_battery_big, LV_ALIGN_CENTER, 0, -20);

      label_backlight_big = lv_label_create(lv_scr_act());
      lv_obj_set_style( label_backlight_big, &st );
      lv_label_set_text(label_backlight_big, "-");
      lv_obj_align(label_backlight_big, LV_ALIGN_CENTER, 0, 50);
