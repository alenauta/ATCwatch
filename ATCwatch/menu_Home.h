...
public:
  virtual void pre()
  {
    ...
    lv_style_set_text_color(&st, LV_STATE_DEFAULT, lv_color_hsv_to_rgb(10, 5, 95));
    st.text.font = &mksd50;

    label_time = lv_label_create(lv_scr_act(), NULL);
    lv_label_set_text_fmt(label_time, "%02i:%02i:%02i", time_data.hr, time_data.min, time_data.sec);
    lv_obj_add_style(label_time, &st);
    ...
    lv_obj_add_style(label_date, &st);
    ...
    lv_style_set_text_color(&style_ble, LV_STATE_DEFAULT, LV_COLOR_BLUE);
    style_ble.text.font = LV_FONT_DEFAULT;
    lv_obj_add_style(label_ble, &style_ble);
    ...
    lv_obj_add_style(label_msg, &style_msg);
  ...
  virtual void main()
  {
    ...
    if (get_vars_ble_connected())
      ...
      lv_style_set_text_color(&style_ble, LV_STATE_DEFAULT, LV_COLOR_MAKE(0x27, 0xA6, 0xFF));
    else
      // style_ble.text.color = LV_COLOR_RED;
      lv_style_set_text_color(&style_ble, LV_STATE_DEFAULT, LV_COLOR_BLUE);
    lv_obj_add_style(label_ble, &style_ble);
    ...
    lv_obj_add_style(label_battery, &style_battery);
  }
